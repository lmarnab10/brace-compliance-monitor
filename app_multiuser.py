...
# ---------------------------------------------------------------
    ...
    # Add a new entry here each time you bring a new device online.
    KNOWN_DEVICES = {
    ...
    device_id = KNOWN_DEVICES[patient_label]
    ...
    # Convert to UTC and remove timezone information so that it can be
    # safely compared with the ESP32 device time.
    ...
    # ESP32/NTP device time.
    # UNSYNCED_xxxxxx values are intentionally treated as invalid device
    # timestamps and will fall back to Supabase server time.
    ...
    device_time = pd.to_datetime(
        wearable_df["device_datetime"].where(
            ~wearable_df["device_datetime"]
            .astype(str)
            .str.startswith("UNSYNCED"),
            None
        ),
        errors="coerce"
    )

    # If ESP32 time is unavailable/UNSYNCED, use the Supabase server time.
    wearable_df["PlotTime"] = device_time.fillna(
        wearable_df["recorded_at"]
    )

    # Remove invalid records and sort chronologically.
    wearable_df = (
        wearable_df
